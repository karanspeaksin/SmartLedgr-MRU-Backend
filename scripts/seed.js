...
async function run() {
  ...
  try {
    ...
    const { rows: [u1] } = await client.query(`
      ...
      VALUES ('John Doe','user2@smartledgr.com',$1,'Admin','JD')
      ON CONFLICT (email) DO UPDATE SET name=EXCLUDED.name RETURNING id`, [hash1]);
    const { rows: [u2] } = await client.query(`
      INSERT INTO users (name, email, password_hash, role, avatar)
      VALUES ('Karan Shah','user5@smartledgr.com',$1,'Manager','KS')
      ON CONFLICT (email) DO UPDATE SET name=EXCLUDED.name RETURNING id`, [hash1]);
    console.log("✓ Users seeded");

    // ── 4. User-Company links (upsert) ─────────────
    await client.query(`INSERT INTO user_companies VALUES ($1,$2,'Admin') ON CONFLICT DO NOTHING`,   [u1.id, comp1]);
    await client.query(`INSERT INTO user_companies VALUES ($1,$2,'Admin') ON CONFLICT DO NOTHING`,   [u1.id, comp2]);
    await client.query(`INSERT INTO user_companies VALUES ($1,$2,'Manager') ON CONFLICT DO NOTHING`, [u2.id, comp1]);
    await client.query(`INSERT INTO user_companies VALUES ($1,$2,'Manager') ON CONFLICT DO NOTHING`, [u2.id, comp2]);
    console.log("✓ User-company links seeded");
    ...
    for (const [code,name,type,taxRate,balance] of accs) {
      await client.query(`INSERT INTO accounts (company_id,code,name,type,tax_rate,balance) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT DO NOTHING`,
    ...
    for (const [name,type,rate] of taxes) {
      await client.query(`INSERT INTO taxes (company_id,name,type,rate) VALUES ($1,$2,$3,$4) ON CONFLICT DO NOTHING`, [comp1,name,type,rate]);
    }
    ...
    for (const [name,accountNo,bank,type,currency,balance] of banks) {
      await client.query(`INSERT INTO bank_accounts (company_id,name,account_no,bank,type,currency,balance) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT DO NOTHING`,
    ...
    for (const [name,email,phone,ar] of customers) {
      await client.query(`INSERT INTO contacts (company_id,type,name,email,phone,ar_balance) VALUES ($1,'customer',$2,$3,$4,$5) ON CONFLICT DO NOTHING`,
    ...
    for (const [name,email,phone,ap] of suppliers) {
      await client.query(`INSERT INTO contacts (company_id,type,name,email,phone,ap_balance) VALUES ($1,'supplier',$2,$3,$4,$5) ON CONFLICT DO NOTHING`,
    ...
    for (const [no,contact,date,status,total] of invoiceData) {
      await client.query(`INSERT INTO invoices (company_id,type,invoice_no,contact_name,date,status,total,currency,created_by) VALUES ($1,'sales',$2,$3,$4,$5,$6,'USD',$7) ON CONFLICT DO NOTHING`,
    ...
    for (const [no,contact,date,status,total] of billData) {
      await client.query(`INSERT INTO invoices (company_id,type,invoice_no,contact_name,date,status,total,currency,created_by) VALUES ($1,'purchase',$2,$3,$4,$5,$6,'USD',$7) ON CONFLICT DO NOTHING`,
        [comp1,no,contact,date,status,total,u1.id]);
    }
    ...
    // ── 10. Cash Books ─────────────────────────────
    const cbData = [
      ["June 2024",8500000,8627217.08,"OPEN"],
      ["May 2024",7200000,8500000,"Reconciled"],
      ["April 2024",6800000,7200000,"Reconciled"],
    ];
    for (const [label,open,close,status] of cbData) {
      await client.query(`INSERT INTO cash_books (company_id,period_label,opening_balance,closing_balance,status,created_by) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT DO NOTHING`,
        [comp1,label,open,close,status,u1.id]);
    }
    console.log("✓ Cash books seeded");

    // ── 11. Journals ───────────────────────────────
    await client.query(`INSERT INTO journals (company_id,journal_no,date,status,method,total_in,created_by) VALUES ($1,'Payroll-14','2026-01-03','PUBLISHED','Accrual and Cash',313000,$2) ON CONFLICT DO NOTHING`, [comp1,u2.id]);
    await client.query(`INSERT INTO journals (company_id,journal_no,date,status,method,total_in,created_by) VALUES ($1,'Payroll-13','2025-12-03','PUBLISHED','Accrual and Cash',313000,$2) ON CONFLICT DO NOTHING`, [comp1,u2.id]);
