// scripts/seed.js — Seeds demo data
...
async function run() {
  ...
  try {
    ...
    // 7. Taxes
    for (const [name,type,rate] of [["VAT 15% (Sales)","Output",15],["VAT 15% (Purchase)","Input",15],["Zero Rated","Exempt",0],["Exempt","Exempt",0]]) {
    ...
    // 8. Bank Accounts
    for (const [name,accountNo,bank,type,currency,balance] of [
    ...
    // 9. Contacts
    for (const [name,email,phone,ar] of [
      ...
      await client.query(`INSERT INTO contacts (company_id,type,name,email,phone,ar_balance) VALUES ($1,'customer',$2,$3,$4,$5)`,
        [comp1,name,email,phone,ar]);
    }
    for (const [name,email,phone,ap] of [
    ...
    // 10. Invoices
    for (const [no,contact,date,status,total] of [
    ...
    // 11. Cash Books
    for (const [label,open,close,status] of [
    ...
    // 12. Journals
    await client.query(`INSERT INTO journals (company_id,journal_no,date,status,method,total_in,created_by) VALUES ($1,'Payroll-14','2026-01-03','PUBLISHED','Accrual and Cash',313000,$2)`, [comp1,u2.id]);
    await client.query(`INSERT INTO journals (company_id,journal_no,date,status,method,total_in,created_by) VALUES ($1,'Payroll-13','2025-12-03','PUBLISHED','Accrual and Cash',313000,$2)`, [comp1,u2.id]);
